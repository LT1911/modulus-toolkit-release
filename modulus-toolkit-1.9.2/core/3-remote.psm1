...
#write-host "Loading \core\3-remote.psm1!" -ForegroundColor Green
...
function Set-WinRM-VM {
    ...
    write-Log "If this function throws errors, try running 'Enable-PSRemoting -Force' first and then try again!" INFO
    ...
    if ($adapter.NetworkCategory -ne "Private") {
        ...
        Write-Log "Updated network adatper MODULUS to use NetworkCategory Private!" INFO
    }

    #Enable-PSRemoting 
    try {
        if (-not (Get-PSSessionConfiguration -name "modulus-PS7" -ErrorAction SilentlyContinue)) {
            Register-PSSessionConfiguration -Name 'modulus-PS7' -StartupScript 'C:\Program Files\PowerShell\Modules\modulus-toolkit\core\startup\1-pssessions.ps1'
            Restart-Service -Name "WinRM" -ErrorAction SilentlyContinue
            Write-Log "'PSSessionConfiguration' registered!"
        } else {
            Unregister-PSSessionConfiguration -Name 'modulus-PS7' -Force
            Register-PSSessionConfiguration -Name 'modulus-PS7' -StartupScript 'C:\Program Files\PowerShell\Modules\modulus-toolkit\core\startup\1-pssessions.ps1'
            Restart-Service -Name "WinRM" -ErrorAction SilentlyContinue
            Write-Log "'PSSessionConfiguration' re-registered!"
        }
    } 
    catch {
        Write-Log "Error (re-)registering 'modulus-PS7' PSSessionConfiguation!" ERROR
    }
    ...
    Write-Log "Adding 3VM scope to trusted users!" INFO
    ...
    if (-not (Get-NetFirewallRule -Name "Allow WinRM" -ErrorAction SilentlyContinue)) {
        ...
        Write-Log "Firewall rule 'Allow WinRM' already exists." WARNING
    }
}
#endregion
...
function Get-Session {
    param (
        [Parameter(Mandatory)]
        [ValidateSet("DB", "APP", "FS")]
        [string]$Server,

        [string]$User = "Administrator",

        [string]$ConfigurationName = 'modulus-PS7',

        [System.Management.Automation.Runspaces.PSSession]$ExistingSession,

        [switch]$ForceNew
    )

    # Reuse existing session if not forcing new
    if (-not $ForceNew -and $ExistingSession -and $ExistingSession.State -eq 'Opened') {
        Write-Log "Using existing session to $Server." -Level WARNING
        return $ExistingSession
    }

    # Determine hostname based on server type
    switch ($Server) {
        "DB"  { $target = Get-MOD-DB-hostname }
        "APP" { $target = Get-MOD-APP-hostname }
        "FS"  { $target = Get-MOD-FS-hostname }
        default {
            Write-Log "Unsupported server type: $Server" -Level ERROR
            throw "Unsupported server type: $Server"
        }
    }

    # Get credentials for the target system
    $cred = Get-DatabaseCredentials -User $User -DB $target

    # Create the session
    $session = New-PSSession -ComputerName $target -Credential $cred -ConfigurationName $ConfigurationName -Authentication Credssp

    Write-Log "New session opened to $Server ($target)." DEBUG
    return $session
}
...
function Close-Session {
    ...
    if ($Session.State -eq 'Opened') {
        ...
        Write-Log "Closing session to $name..." -Level DEBUG
        Remove-PSSession $Session
    } else {
        Write-Log "Session is already closed or not valid." -Level DEBUG
    }
}
